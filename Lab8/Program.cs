...
var integrator = new RectangleIntegrator(a, b, 10, epsilon, RectangleIntegrator.MethodType.Parabola,function);
...
foreach (var iteration in integrator.Iterations)
{
    table1.Rows.Add(k, iteration.N, iteration.H, iteration.I, last is null ? "—" : Math.Abs(iteration.I - last.I));
    last = iteration;
    k++;
}
table1.CalcWidth();
